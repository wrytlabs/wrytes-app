...
export const AuthorizationOverview: React.FC<AuthorizationOverviewProps> = ({
  ...
  return (
    <Card>
      <h3 className="text-lg font-semibold text-white mb-4">Authorized Addresses</h3>
      <p className="text-text-secondary text-sm mb-6">
        Select an address to create authorization signatures. Connected wallets have full access,
        while authorized addresses can sign on behalf of contracts and smart wallets.
      </p>
      ...
      <div className="space-y-3 overflow-hidden">
        {addresses.map(address => (
          <div
            ...
            className={`
                  p-4 rounded-lg bg-dark-surface border cursor-pointer transition-all duration-200 min-w-0 w-full
                  ${
          ...
          >
            <div className="flex items-center justify-between min-w-0">
              ...
              <div className="text-right flex-shrink-0 ml-2 sm:ml-3">
                <span
                  className={`
                      inline-flex items-center px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-md text-xs font-medium
                      ${
                        address.isConnected
                ...
                >
                  ...
                  <span className="sm:hidden">{address.isConnected ? 'C' : 'A'}</span>
                </span>
              </div>
            </div>
          </div>
